...
(use-package prog-mode :ensure nil
  :hook (prog-mode . hs-minor-mode) (prog-mode . outline-minor-mode))
#+end_src
**** electric-pair
...
(use-package erc :ensure nil
  ...
  (defun erc-run-libera ()
    ...
    (interactive)
    (erc-tls :server "irc.libera.chat" :nick "zororg")
    )
...
- more additional hyprland conf for quick reloads
  ...
  # exec-once = swayidle -w timeout 300 'gtklock -i -b black -d' timeout 210 'hyprctl dispatch dpms off' resume 'hyprctl dispatch dpms on' timeout 302 'if pgrep gtklock; then hyprctl dispatch dpms off; fi' resume 'hyprctl dispatch dpms on' timeout 1000 'systemctl suspend'
