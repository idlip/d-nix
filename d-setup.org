...
#+begin_src js :tangle ~/.config/glide/glide.ts
  ...
  function setup_autocommands() {
      glide.autocmds.create("ModeChanged", "*", (args) => {
          if (args.new_mode === "visual") {
              glide.excmds.execute("mode_change normal");
              return;
          }
          const colors: Record<keyof GlideModes, string> = {
          ...
          glide.styles.add(
              `#browser { border-bottom: 3px solid var(${colors[args.new_mode]}) }`,
  ...
  #+begin_src elisp :tangle ~/.config/noctalia/templates/emacs.el
    ...
    (setq
     ...
     '(
       ...
       ;; UI elements
       (fringe         unspecified)
       (bg-mode-line-active "{{colors.surface_dim.default.hex}}")
       ))
