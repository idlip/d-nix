...
#+begin_src nix :noweb-ref os-gdk-fs
  {
    fileSystems."/" = { device = "/dev/disk/by-uuid/209f8b3a-b7ae-44aa-bd18-b9f8965e57ab"; };
    fileSystems."/home" = { device = "/dev/disk/by-uuid/209f8b3a-b7ae-44aa-bd18-b9f8965e57ab"; };
    fileSystems."/nix" = { device = "/dev/disk/by-uuid/209f8b3a-b7ae-44aa-bd18-b9f8965e57ab"; };
    fileSystems."/boot" = { device = "/dev/disk/by-uuid/C30A-2932"; };
  }
#+end_src
...
#+begin_src js :tangle ~/.config/glide/glide.ts
  ...
  async function showLinkPicker(openInNewTab: boolean = false) {
    const currentTab = await getCurrentTab();
    if (!currentTab.id) return;

    let links: Array<{ text: string; href: string; xpath: string }>;

    try {
      links = await glide.content.execute(() => {
        const results: Array<{ text: string; href: string; xpath: string }> = [];
        const MAX = 1000;
        ...
        const getXPath = (el: Element): string => {
          ...
          while (sib) {
            if (sib.nodeType === 1 && (sib as Element).tagName === el.tagName) idx++;
    ...
    glide.commandline.show({
      title: `${openInNewTab ? 'Open Link in New Tab' : 'Open Link'} (${links.length})`,
      options: links.map((link) => ({
        label: link.text,
        description: link.href,
        matches({ input }) {
          return orderlessMatch(`${link.text} ${link.href}`, input);
        },
        async execute() {
          if (openInNewTab) {
  ...
  const URL_RULES: URLRule[] = [
    // Redirects
    { domain: 'reddit.com', action: 'redirect', to: 'https://libreddit.kavin.rocks$1' },
    ...
    // Path-specific blocks (uncomment to use)
    ...
    try {
      ...
      for (const rule of URL_RULES) {
        ...
        if (rule.action === 'redirect' && rule.to) {
          ...
          const newUrl = rule.to.replace('$1', path);
  ...
  function setupRedirects() {
    ...
    if (browser.webRequest.onBeforeRequest.hasListener(handleURLRules)) {
  ...
  function setupKeymaps() {
    ...
    // Visual mode
    glide.keymaps.set(["normal", "insert"], "<C-Space>", "motion v");
    glide.keymaps.set("visual", "<C-b>", "motion vh");
    glide.keymaps.set("visual", "<C-f>", "motion vl");
    glide.keymaps.set("visual", "d", "motion vd");
    glide.keymaps.set("visual", "c", "motion vc");
    glide.keymaps.set("visual", "<A-w>", "visual_selection_copy");
    ...
    glide.keymaps.set(["normal", "insert"], "<A-o>", () =>
  ...
  async function showCommandPalette() {
    ...
    // Group by mode
    const byMode: Record<string, typeof allKeymaps> = {};
    allKeymaps.forEach(km => {
      if (!byMode[km.mode]) byMode[km.mode] = [];
      byMode[km.mode].push(km);
    });
...
** Init
:PROPERTIES:
:header-args:emacs-lisp:  :tangle gdk/configs/d-emacs/init.el
:END:
  ...
  #+begin_src elisp :tangle ~/.config/noctalia/templates/emacs.el
    ...
    (setq
     ...
     '(
       ...
       ;; Tab bar
       (bg-tab-bar     "{{colors.surface.default.hex}}")
       (bg-tab-current "{{colors.surface_container_low.default.hex}}")
       (bg-tab-other   "{{colors.surface_dim.default.hex}}")
       ...
       ;; Line numbers
       (bg-line-number-active    "{{colors.surface.default.hex}}")
       (bg-line-number-inactive  "{{colors.surface.default.hex}}")
       (fg-line-number-active    "{{colors.on_surface_variant.default.hex}}")
       (fg-line-number-inactive  "{{colors.outline_variant.default.hex}}")

       ;; Borders
       (border-mode-line-active   "{{colors.secondary.default.hex}}")
