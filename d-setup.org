...
** Hosts & FIles in flakes
:PROPERTIES:
:header-args: :noweb-ref flake-files
:END:
...
#+begin_src json-ts :noweb-ref noctalia :tangle ~/.config/noctalia/settings.json 
  {
    ...
    "screenRecorder": {
      ...
      "directory": "/home/dev/media/stream",
      "frameRate": 60,
      "quality": "very_high",
      "showCursor": true,
      "videoCodec": "h264",
...
#+begin_src typescript :tangle ~/.config/glide/glide.ts
  ...
  const allModes: GlideModes[keyof GlideModes][] = [
    "normal",
    "insert",
    "visual",
    "ignore",
  ...
  glide.keymaps.set(["normal", "insert"], "<C-x>", "");

  glide.o.hint_size = "18px";
  glide.o.hint_chars = "asdfhjklnpw"; // we will use custom hint generator
  glide.o.which_key_delay = 100;
  glide.o.mapping_timeout = 10000;

  glide.prefs.set("sidebar.animation.expand-on-hover.duration-ms", 10);
  glide.prefs.set("toolkit.legacyUserProfileCustomizations.stylesheets", true);

  glide.prefs.set("font.name.monospace.x-western", "Ioskeley Mono");
  glide.prefs.set("font.name.sans-serif.x-western", "Inter");
  glide.prefs.set("font.name.serif.x-western", "Inter");

  // firefox sync
  glide.prefs.set("identity.fxaccounts.enabled", true);

  // Fill SVG Color
  glide.prefs.set("svg.context-properties.content.enabled", true);

  // CSS's `:has()` selector
  glide.prefs.set("layout.css.has-selector.enabled", true);

  // Integrated calculator at urlbar
  glide.prefs.set("browser.urlbar.suggest.calculator", true);

  // Integrated unit convertor at urlbar
  glide.prefs.set("browser.urlbar.unitConversion.enabled", true);
  glide.prefs.set("browser.tabs.insertAfterCurrent", true);

  // Trim  URL
  glide.prefs.set("browser.urlbar.trimHttps", true);
  glide.prefs.set("browser.urlbar.trimURLs", true);

  // GTK rounded corners
  glide.prefs.set("widget.gtk.rounded-bottom-corners.enabled", true);

  glide.prefs.set("layout.css.grid-template-masonry-value.enabled", true);
  glide.prefs.set("dom.enable_web_task_scheduling", true);
  ...
  glide.prefs.set("signon.generation.enabled", false);

  glide.prefs.set("layout.css.prefers-color-scheme.content-override", 2);
  glide.prefs.set("browser.compactmode.show", true);

  /** URL BAR ***/
  glide.prefs.set("browser.urlbar.suggest.engines", false);
  ...
  glide.prefs.set("media.autoplay.blocking_policy", 0);

  /** PASSWORDS ***/
  glide.prefs.set("editor.truncate_user_pastes", false);

  /** DOWNLOADS ***/
  glide.prefs.set("browser.download.autohideButton", true);

  /** PDF ***/
  glide.prefs.set("browser.download.open_pdf_attachments_inline", true);

  /** TAB BEHAVIOR ***/
  glide.prefs.set("browser.tabs.loadBookmarksInTabs", true);
  glide.prefs.set("browser.bookmarks.openInTabClosesMenu", false);
  glide.prefs.set("findbar.highlightAll", true);
  ...
  // glide.keymaps.set("normal", "<C-x><C-v>", "editURL");

  glide.keymaps.set("normal", "<A-<>", "scroll_top");
  glide.keymaps.set("normal", "<A-]>", "scroll_bottom");
  glide.keymaps.set(["normal", "insert", ], "<C-y>", () => glide.keys.send("<C-v>", { skip_mappings: true }));
  glide.keymaps.set(["normal", "visual", ], "<A-v>", () => glide.keys.send("<pageup>", { skip_mappings: true }));
  glide.keymaps.set(["normal", "visual", ], "<C-v>", () => glide.keys.send("<pagedown>", { skip_mappings: true }));


  // ignore mode
  glide.keymaps.set(["normal", "insert", "visual", "command"], "<C-g>", "mode_change ignore");
  glide.keymaps.set(allModes, "<C-q>", );
  glide.keymaps.set("ignore", "<C-g>", "mode_change normal");
  glide.keymaps.set(["insert", "command", "ignore"], "<C-x><C-q>", "mode_change normal");
  glide.keymaps.set(["visual", "normal"], "<C-x><C-q>", "mode_change insert");
  ...
  glide.keymaps.set("hint", ["<C-g>", "<escape"], "hints_remove");
  ...
  // map emacs keybinding to firefox native keybindings
  glide.keymaps.set(["normal", "insert"], "<C-x><C-f>", () => glide.keys.send('<C-l>', { skip_mappings: true }));
  glide.keymaps.set(["normal", "insert", "ignore"], "<C-s>", () => glide.keys.send('<C-f>', { skip_mappings: true }));

  glide.keymaps.set(["normal", "insert"], "<C-g>", () => glide.keys.send('<Esc>'));

  glide.keymaps.set(["normal", "insert", "visual", "ignore"], "<C-n>", () => glide.keys.send("<Down>", { skip_mappings: true }));
  glide.keymaps.set(["normal", "insert", "visual", "ignore"], "<C-p>", () => glide.keys.send("<Up>", { skip_mappings: true }));
  // glide.keymaps.set("normal", "<C-n>", "caret_move down");
  // glide.keymaps.set("normal", "<C-p>", "caret_move up");

  glide.keymaps.set("normal", "<C-x>z", "repeat");
  glide.keymaps.set(["normal","visual","insert"], "<A-x>", "commandline_toggle");
  glide.keymaps.set("normal", ":", );
  glide.keymaps.set("normal", "<A>", "commandline_toggle");
  ...
  glide.keymaps.set(["normal", "visual", "insert"], "<A-h>", () => glide.keys.send("<C-a>", { skip_mappings: true }));
  glide.keymaps.set(["normal", "visual", "insert"], "<C-x>h", () => glide.keys.send("<C-a>", { skip_mappings: true }));
  ...
  // credits: @jyn514
  // https://github.com/glide-browser/glide/discussions/147#discussioncomment-15286767
  ...
  /**
   ...
   ,*/
  async function swap_to_tab_if_exists(url: string): Promise<void> {
  ...
  /**
   ...
   ,*/
  function fuzzy_score(haystack: string, needle: string): number {
    if (!needle) return 1;
  ...
  /**
   ,* Create a favicon + title + url display element
   ,*/
  function create_item_display(item: { url?: string; title?: string; visitCount?: number }): HTMLElement {
  ...
  /**
   ,* C-x C-v: Current site history picker
   ,*/
  glide.keymaps.set(["normal", "insert"], "<C-x><C-v>", async () => {
    ...
    const unique = sameOrigin.filter(item => {
    ...
    glide.commandline.show({
      ...
      filter(input, option) {
        const item = unique.find(e => e.url === option.value);
        if (!item) return false;

        const titleScore = fuzzy_score(item.title || "", input);
        const urlScore = fuzzy_score(item.url || "", input) * 0.5;

        return (titleScore + urlScore) > 0;
      },
      options: unique.map(item => ({
        label: item.title || item.url || "",
        value: item.url,
        render: () => create_item_display(item),
        async execute() {
          if (item.url) await swap_to_tab_if_exists(item.url);
        },
      })),
    });
  }, { description: "Fuzzy find current site history" });


  glide.keymaps.set(["normal", "insert"], "<A-o>", async () => {
    const allHistory = await browser.history.search({
      text: "",
      startTime: 0,
      maxResults: 50000
    ...
    // Group by domain
    const byDomain = new Map<string, typeof allHistory>();
    ...
    glide.commandline.show({
      title: `All History (${allHistory.length} items)`,
      options: allHistory.slice(0, 4000).map((item) => ({
        label: item.title || item.url,
        render: () => create_item_display(item),
        async execute() {
          await swap_to_tab_if_exists(item.url);
        },
      })),
    });
  }, { description: "Show all raw history" });
#+end_src
...
**** userChrome css
:PROPERTIES:
:header-args: :noweb-ref userchrome
:END:
