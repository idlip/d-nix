...
#+begin_src js :tangle ~/.config/glide/glide.ts
  ...
  const ALL_MODES: GlideModes[keyof GlideModes][] = [
    "normal", "insert", "visual", "ignore", "command", "op-pending"
  ];
  ...
  function orderlessMatch(haystack: string, needle: string): boolean {
  ...
  async function swapToTabIfExists(url: string): Promise<void> {
  ...
  async function showTabPicker(action: 'close' | 'discard', filterFn?: (tab: browser.tabs.Tab) => boolean) {
    ...
    glide.commandline.show({
      ...
      options: filtered.map((tab) => ({
        ...
        description: tab.url,
        matches({ input }) {
          return orderlessMatch(`${tab.title} ${tab.url}`, input);
        },
        async execute() {
          ...
          if (action === 'close') {
            ...
            if (currentTab.id === tab.id) {
              ...
              const idx = allTabs.findIndex(t => t.id === currentTab.id);
  ...
  async function showSplitViewPicker(tab_id: number) {
  ...
  async function showHistoryPicker(currentSiteOnly: boolean = false) {
    const maxResults = currentSiteOnly ? 500 : 1000;
    const allHistory = await browser.history.search({ text: "", startTime: 0, maxResults });

    let filtered = allHistory;
    let title = `History (${allHistory.length})`;
    ...
    if (currentSiteOnly) {
      ...
      const origin = (() => {
        try { return new URL(activeTab.url).origin; }
        catch { return null; }
      })();
      if (!origin) return;
    ...
    glide.commandline.show({
      ...
      options: filtered.slice(0, 500).map((item) => ({
        ...
        matches({ input }) {
          return orderlessMatch(`${item.title} ${item.url}`, input);
        },
        async execute() {
          await swapToTabIfExists(item.url);
        },
      })),
    });
  }
  ...
  async function showRecentlyClosedPicker() {
  ...
  async function openUrl(url: string, openInNewTab: boolean, cookieStoreId?: string) {
    if (cookieStoreId) {
      await browser.tabs.create({ url, cookieStoreId } as any);
  ...
  async function showUniversalOpen(openInNewTab = false, cookieStoreId?: string, onSelect?: (url: string) => Promise<void>) {
    const [history, allTabs] = await Promise.all([
    ...
    const go = async (url: string, input?: string) => {
      if (onSelect) return onSelect(url);
      await openUrl(url, openInNewTab, cookieStoreId);
    };

    // Always-visible search fallback — executes on current input
    options.push({
      label: 'Search…',
    ...
    // Open tabs — switch if same-tab open, else navigate/container
    for (const tab of allTabs) {
      ...
      options.push({
        ...
        async execute() {
          if (onSelect) return onSelect(tab.url!);
          if (cookieStoreId || openInNewTab) {
            await openUrl(tab.url!, openInNewTab, cookieStoreId);
          } else {
            await swapToTabIfExists(tab.url!);
          }
        },
      });
    }
    ...
    const containerLabel = cookieStoreId ? ' [container]' : '';
    const mode = openInNewTab ? 'New Tab' : 'Open';
    glide.commandline.show({
      title: `${mode}${containerLabel} (${options.length - 1})`,
      options,
    });
  }

  async function showContainerOpen() {
    await showUniversalOpen(false, undefined, async (url) => {
      const ci = (browser as any).contextualIdentities;
      const win = await browser.windows.getCurrent();

      // Private window or no API — open directly, skip container picker
      if (win.incognito || !ci) {
        await browser.tabs.create({ url, active: true });
        return;
      }
      ...
      const normalWindowId = wins.find(w => !w.incognito)?.id;
      const createIn = (csId?: string) =>
        (browser.tabs.create as any)({ url, active: true, windowId: normalWindowId, ...(csId && { cookieStoreId: csId }) });

      glide.commandline.show({
        title: `Container for: ${url.length > 50 ? url.slice(0, 50) + '…' : url}`,
        options: [
          { label: 'Default', description: 'no container',
            matches({ input }: any) { return orderlessMatch('default', input); },
            async execute() { await createIn(); } },
          ...containers.map((c: any) => ({
            label: c.name, description: c.color,
            matches({ input }: any) { return orderlessMatch(c.name, input); },
            async execute() { await createIn(c.cookieStoreId); },
          })),
        ],
      });
    });
  ...
  async function showLinkPicker(openInNewTab: boolean = false, yank: boolean = false) {
    ...
    try {
      links = await glide.content.execute(() => {
        ...
        // Walk ancestors, subtract link text → get surrounding context only
        const getContext = (el: Element): string => {
          const linkText = clean(el.textContent || '');
          let node: Element | null = el.parentElement;
          for (let i = 0; i < 5 && node; i++) {
            const ctx = clean(node.textContent || '').replace(linkText, '').trim();
        ...
        // Anchor links — deduplicated by href, aria-label/title fallback for icon-only links
        for (const el of document.querySelectorAll('a[href]')) {
          ...
          results.push({ text, href: a.href, context: getContext(a) });
        }

        // Plain text URLs in text nodes (capped to avoid slowdowns on large pages)
        ...
        while ((node = walker.nextNode()) && scanned < 500 && results.length < MAX) {
          scanned++;
          for (const match of (node.textContent || '').matchAll(urlRe)) {
    ...
    const title = yank ? 'Yank Link' : openInNewTab ? 'Open Link in New Tab' : 'Open Link';
    glide.commandline.show({
      ...
      options: links.map((link) => ({
        ...
        matches({ input }) {
          return orderlessMatch(`${link.text} ${link.context} ${link.href}`, input);
        },
        async execute() {
          if (yank) {
            await glide.content.execute(
              (href: string) => navigator.clipboard.writeText(href),
              { tab_id: currentTab.id, args: [link.href] }
            );
          } else if (openInNewTab) {
            await browser.tabs.create({ url: link.href });
          } else {
            await browser.tabs.update(currentTab.id, { url: link.href });
          }
        },
      })),
    });
  ...
  // ============================================================================
  // PAGE NAVIGATOR - Navigate page structure and search text
  // ============================================================================

  async function showPageNavigator() {
    const currentTab = await getCurrentTab();
    if (!currentTab.id) return;
    ...
    try {
      items = await glide.content.execute(() => {
        ...
        // Text content (paragraphs, list items)
        const textSelector = 'p, li, dt, dd, figcaption, blockquote';
    ...
    glide.commandline.show({
      ...
      options: items.map((item) => ({
        ...
        matches({ input }) {
          return orderlessMatch(item.text, input);
        },
        async execute() {
          ...
          }, { tab_id: currentTab.id, args: [item.xpath] });
        },
      })),
    });
  }
  ...
  const URL_RULES: URLRule[] = [
    // Redirects — replace $1 with full path+search+hash
  ...
  // Version stamp — written to env on each config load.
  // Old listeners from previous reloads read a stale version and become no-ops,
  // since hasListener() can't find old function refs across sandbox re-evaluations.
  const _RULES_VER = String(Date.now());
  const _RULES_VER_KEY = 'GLIDE_URL_RULES_VER';

  function handleURLRules(
    details: browser.webRequest.OnBeforeRequestDetailsType
  ): browser.webRequest.BlockingResponse {
    // Stale listener from a previous config reload — skip
    ...
    try {
      ...
      for (const rule of URL_RULES) {
        ...
        if (rule.action === 'block') {
          // Redirect to blank rather than cancel — avoids Firefox showing
          // a network error page that offers "try original URL" fallback.
          return { redirectUrl: 'about:blank' };
        }

        if (rule.action === 'redirect' && rule.to) {
          // Loop guard: if we're already on the redirect target host, skip.
          try {
            const targetUrl = new URL(rule.to.replace('$1', '/'));
  ...
  function setupRedirects() {
    // Stamp this version so old listeners from prior reloads neutralize themselves.
    glide.env.set(_RULES_VER_KEY, _RULES_VER);
    // removeListener is safe even if fn not registered (different ref = no-op).
    browser.webRequest.onBeforeRequest.removeListener(handleURLRules);
  ...
  function setupStyles() {
    glide.styles.add(css`
      ...
      /* URL column background color for custom completions */
      ...
      /* widen label column, dim description when not focused */
  ...
  function setupPrefs() {
    const prefs: Record<string, any> = {
      ...
      // "toolkit.scrollbox.pagescroll.maxOverlapLines": 50, // we got scroll_half now
  ...
  function setupKeymaps() {
    ...
    glide.keymaps.set(["normal", "insert", "visual", "command"], "<C-q>", "mode_change normal");
    ...
    // Tab close/discard pickers
    glide.keymaps.set(["normal", "insert"], "<C-x>k", () =>
      showTabPicker("close"),
    );
    glide.keymaps.set(["normal", "insert"], "<A-u>", "tab_discard");
    glide.keymaps.set(["normal", "insert"], "<A-U>", () =>
      showTabPicker("discard", (tab) => !tab.discarded),
    );

    // Scrolling
    glide.keymaps.set("normal", "<A-[>", "scroll_top");
    glide.keymaps.set("normal", "<A-]>", "scroll_bottom");
    glide.keymaps.set(["normal", "visual"], "<A-v>", "scroll_half_page_up");
    glide.keymaps.set(["normal", "visual"], "<C-v>", "scroll_half_page_down");
    glide.keymaps.set(["normal", "visual"], "^", "go_up");
    glide.keymaps.set(["normal", "visual"], "]", "go_next");
    glide.keymaps.set(["normal", "visual"], "[", "go_previous");
    ...
    glide.keymaps.set(["normal", "insert", "visual", "ignore"], "<C-n>", () =>
      glide.keys.send("<Down>", { skip_mappings: true }),
    );
    glide.keymaps.set(["normal", "insert", "visual", "ignore"], "<C-p>", () =>
    ...
    glide.keymaps.set(["normal", "visual", "insert"], "<A-h>", () =>
      glide.keys.send("<C-a>", { skip_mappings: true }),
    );
    glide.keymaps.set(["normal", "visual", "insert"], "<C-x>h", () =>
      glide.keys.send("<C-a>", { skip_mappings: true }),
    );
    glide.keymaps.set(["normal", "insert", "visual"], "<C-a>", "motion 0");
    glide.keymaps.set(["normal", "insert", "visual"], "<C-e>", "motion $");
    glide.keymaps.set(["normal", "insert"], "<C-b>", "caret_move left");
    glide.keymaps.set(["normal", "insert"], "<C-f>", "caret_move right");

    // Word motions
    glide.keymaps.set(["visual", "insert"], "<A-f>", "motion e");
    glide.keymaps.set(["visual", "insert"], "<A-b>", "motion b");
    glide.keymaps.set(["visual", "insert"], "<C-d>", "motion x");
    ...
    // Split views
    glide.keymaps.set("normal", "<C-x>2", ({ tab_id }) =>
      showSplitViewPicker(tab_id),
    );
    ...
    // Link pickers
    glide.keymaps.set(ALL_MODES, "<C-.>", () => showLinkPicker(false));
    glide.keymaps.set("normal", "<A-.>", () => showLinkPicker(true));
    glide.keymaps.set("normal", "<leader>y", () => showLinkPicker(false, true));
    glide.keymaps.set(ALL_MODES, "<A-y>", () => showLinkPicker(false, true));

    glide.keymaps.set("normal", "<A-s>", () => showPageNavigator());
    glide.keymaps.set("normal", "<A-g>", () => showPageNavigator());

    // History pickers
    glide.keymaps.set(["normal", "insert"], "<C-x><C-v>", () =>
      showHistoryPicker(true),
    );

    // Universal open: URL first, then optionally pick container
    glide.keymaps.set(["normal", "insert"], "<A-o>", () => showContainerOpen());
    glide.keymaps.set(["normal", "insert"], "<A-O>", () => showUniversalOpen(true));

    // Recently closed tabs
    glide.keymaps.set(["normal", "insert"], "<A-r>", () => showRecentlyClosedPicker());
  }

  function setupExcmds() {
    glide.excmds.create(
      { name: "tab_discard", description: "Discard tab" },
      async ({ tab_id }) => {
  ...
  glide.keymaps.set("normal", "<C-h>k", () => showCommandPalette());

  async function showCommandPalette() {
    const allKeymaps = glide.keymaps.list();
    const commands = allKeymaps.map(km => {
    ...
    glide.commandline.show({
      ...
      options: commands.map((cmd) => ({
        ...
        matches({ input }) {
          const searchText = `${cmd.mode} ${cmd.lhs} ${cmd.action} ${cmd.description}`.toLowerCase();
          return orderlessMatch(searchText, input);
        },
        async execute() {
          // Execute the command
  ...
  function init() {
    ...
    setupKeymaps();
    setupAutocommands();
    glide.autocmds.create("ConfigLoaded", setupRedirects);  
  }
