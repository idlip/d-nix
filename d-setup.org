...
#+begin_src typescript :tangle ~/.config/glide/glide.ts
  ...
  /**
   ,* Fuzzy matching score for search/filter
   ,*/
  function fuzzyScore(haystack: string, needle: string): number {
    ...
    if (hay === need) return 1000;
    if (hay.startsWith(need)) return 500;
    if (hay.includes(need)) return 100;

    let score = 0;
    let lastIndex = -1;
    for (const char of need) {
      const index = hay.indexOf(char, lastIndex + 1);
      if (index === -1) return 0;
      score += 1;
      lastIndex = index;
    }
  ...
  // Add after the fuzzyScore function in UTILITIES section

  /**
   ,* Orderless matching: check if all words in needle appear in haystack (any order)
   ,*/
  ...
  /**
   ,* Score for orderless matching (higher = better match)
   ,*/
  function orderlessScore(haystack: string, needle: string): number {
    if (!needle.trim()) return 1;
    if (!orderlessMatch(haystack, needle)) return 0;
    ...
    words.forEach((word) => {
      if (hay.startsWith(word)) score += 50;
      else if (new RegExp(`\\b${word}`).test(hay)) score += 25;
    });

    // Penalty for distance between matched words
  ...
  /**
   ,* Create favicon + title + url display element as table row
   ,*/
  ...
  /**
   ,* Swap to existing tab or create new one
   ,*/
  ...
  // ============================================================================
  // HINT LABEL GENERATION (credits: @jyn514)
  // ============================================================================

  function shortenUniquePrefixes(haystack: string[]): string[] {
    const seen = new Set();
    ...
    return haystack.map((needle, pos) => {
      ...
      for (let i = 1; i <= needle.length; i++) {
        ...
        const is_unique = haystack.every(
          (w, j) => pos === j || !w.startsWith(prefix),
        );
  ...
  function generateLabels(texts: string[]): string[] {
    const haystack = shortenUniquePrefixes(texts);
    let nums_used = 0;
    ...
    return shortenUniquePrefixes(result);
  }
  ...
  type ClickableAction = 'click' | 'open' | 'newtab' | 'background' | 'copy';

  async function showClickablePicker(action: ClickableAction = 'click') {
    const currentTab = await getCurrentTab();
    if (!currentTab.id) return;

    let clickables: Array<{
      type: string;
      text: string;
      href?: string;
      xpath: string;
    ...
    try {
      // Use a simpler, faster query - just get essential data
      clickables = await glide.content.execute(() => {
        const results: Array<{
          type: string;
          text: string;
          href?: string;
        ...
        // Fast xpath generator using element ID if available
        const getXPath = (element: Element): string => {
          ...
          for (; el && el.nodeType === 1 && count < 5; el = el.parentElement) {
        ...
        // Just links - fastest and most common use case
        if (count < MAX_ITEMS) {
          const links = document.querySelectorAll('a[href]');
          const len = Math.min(links.length, MAX_ITEMS);
          for (let i = 0; i < len; i++) {
        ...
        if (count < MAX_ITEMS) {
          ...
          const len = Math.min(buttons.length, MAX_ITEMS - count);
          for (let i = 0; i < len; i++) {
            const el = buttons[i];
            const text = getText(el);
            if (text) {
      ...
      console.error('Clickable picker error:', error);
      glide.commandline.show({
        title: "Error: Could not extract clickable elements",
        options: []
      });
      return;
    }

    const actionTitles: Record<ClickableAction, string> = {
      click: "Click Element",
      open: "Open Link",
    ...
    glide.commandline.show({
      title: `${actionTitles[action]} (${clickables.length} found)`,
      options: clickables.map((item) => ({
        label: item.text,
        description: item.href || item.type,
        matches({ input }) {
          const searchText = `${item.text} ${item.href}`.toLowerCase();
          return orderlessMatch(searchText, input);
        },
        async execute() {
          if (action === 'click' || (action === 'open' && !item.href)) {
            await glide.content.execute((xpath: string) => {
              const result = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
              const target = result.singleNodeValue as HTMLElement;
              if (target) target.click();
            }, { tab_id: currentTab.id, args: [item.xpath] });
          } else if (item.href) {
            switch (action) {
              case 'open':
                await browser.tabs.update(currentTab.id, { url: item.href });
                break;
              case 'copy':
                await navigator.clipboard.writeText(item.href);
                break;
              case 'newtab':
                await browser.tabs.create({ url: item.href });
                break;
              case 'background':
                await browser.tabs.create({ url: item.href, active: false });
                break;
            }
          }
        },
  ...
  /**
   ,* Show semantic element picker to jump to any important page element
   ,*/
  async function showPageNavigator() {
    const currentTab = await getCurrentTab();
    if (!currentTab.id) return;

    const elements = await glide.content.execute(() => {
      const results: Array<{
        type: string;
        tag: string;
        text: string;
      ...
      const getTextPreview = (el: Element, maxLength: number = 80): string => {
      ...
      document.querySelectorAll('header, nav, main, article, section, aside, footer').forEach(el => {
        results.push({
          type: 'landmark',
          tag: el.tagName.toLowerCase(),
          text: getTextPreview(el, 60),
          id: el.id || '',
          class: el.className || '',
          xpath: getXPath(el)
        });
      ...
      // Lists (only top-level ul/ol with multiple items)
      document.querySelectorAll('ul, ol').forEach(el => {
        if (el.parentElement?.closest('ul, ol')) return;
        ...
        if (items.length > 0) {
          results.push({
            type: 'list',
            tag: el.tagName.toLowerCase(),
            text: `${items.length} items: ${getTextPreview(items[0], 50)}`,
            id: el.id || '',
            xpath: getXPath(el)
          });
        }
      ...
      // Images with alt text or meaningful src
      document.querySelectorAll('img').forEach(el => {
        ...
        if (img.alt || img.width > 100 || img.height > 100) {
          results.push({
            type: 'image',
            tag: 'img',
            text: img.alt || img.title || img.src.split('/').pop() || 'Image',
            src: img.src,
            xpath: getXPath(el)
          });
        }
      ...
      // Links with aria-label or meaningful text
      document.querySelectorAll('a[aria-label], a[role="button"]').forEach(el => {
        const link = el as HTMLAnchorElement;
        results.push({
          type: 'link',
          tag: 'a',
          text: el.getAttribute('aria-label') || getTextPreview(el, 50),
          href: link.href,
          xpath: getXPath(el)
        });
      });
      ...
      document.querySelectorAll('form').forEach(el => {
        ...
        results.push({
          ...
          id: el.id || '',
          name: (el as HTMLFormElement).name || '',
          xpath: getXPath(el)
        });
      });
      ...
      document.querySelectorAll('table').forEach(el => {
        ...
        results.push({
          type: 'table',
          tag: 'table',
          text: caption || `Table with ${rows} rows`,
          id: el.id || '',
          xpath: getXPath(el)
        });
      });
      ...
      // ARIA landmarks
      document.querySelectorAll('[role="banner"], [role="navigation"], [role="main"], [role="complementary"], [role="contentinfo"], [role="search"]').forEach(el => {
        results.push({
          type: 'aria',
          tag: el.tagName.toLowerCase(),
          role: el.getAttribute('role') || '',
          text: el.getAttribute('aria-label') || getTextPreview(el, 60),
          xpath: getXPath(el)
        });
      });
    ...
    if (!elements || elements.length === 0) {
      glide.commandline.show({
        title: "No navigable elements found",
        options: []
      });
      return;
    }
    ...
    const typeOrder = ['landmark', 'heading', 'aria', 'list', 'form', 'table', 'image', 'button', 'link'];
    elements.sort((a, b) => {
    ...
    glide.commandline.show({
      title: `Navigate Page (${elements.length} elements)`,
      options: elements.map((element) => {
        const icon = typeIcons[element.type] || '•';
        const indent = element.type === 'heading' && element.level
          ? '  '.repeat(element.level - 1)
          : '';

        let label = '';
        if (element.type === 'heading') {
          label = `${indent}${element.tag.toUpperCase()}: ${element.text}`;
        } else if (element.type === 'landmark') {
          label = `${icon} <${element.tag}> ${element.text}`;
        } else if (element.type === 'aria') {
          label = `${icon} [${element.role}] ${element.text}`;
        } else {
          label = `${icon} ${element.text}`;
        }

        let description = '';
        if (element.id) description = `#${element.id}`;
        else if (element.class) description = `.${element.class.split(' ')[0]}`;
        else if (element.href) description = element.href;
        else if (element.src) description = element.src;

        return {
          label,
          description,
          matches({ input }) {
            const searchText = `${element.type} ${element.tag} ${element.text} ${element.id || ''} ${element.role || ''}`.toLowerCase();
            return orderlessMatch(searchText, input);
          },
          async execute() {
            await glide.content.execute((xpath: string) => {
              const getElementByXPath = (xp: string) => {
            ...
            }, { tab_id: currentTab.id, args: [element.xpath] });
          },
        };
      }),
    });
  }
  async function showLineNavigator() {
    const currentTab = await getCurrentTab();
    if (!currentTab.id) return;
    ...
    try {
      lines = await glide.content.execute(() => {
        const results: Array<{
          text: string;
          lineNumber: number;
          xpath: string;
        ...
        const walker = document.createTreeWalker(
          ...
          NodeFilter.SHOW_TEXT,
          {
            acceptNode: (node) => {
              ...
              // Skip empty, very short, or script/style content
      ...
      console.error('Line navigator error:', error);
      glide.commandline.show({
        title: "Error: Could not extract text content",
        options: []
      });
      return;
    }
    ...
    glide.commandline.show({
      title: `Search Lines (${lines.length} found)`,
      options: lines.map((line) => ({
        label: `Line ${line.lineNumber}`,
        description: line.text,
        matches({ input }) {
          return orderlessMatch(line.text, input);
        },
        async execute() {
          await glide.content.execute((xpath: string) => {
            const getElementByXPath = (xp: string) => {
            ...
            if (target) {
              ...
              target.style.backgroundColor = 'rgba(255, 165, 0, 0.3)';
              setTimeout(() => { target.style.backgroundColor = originalBg; }, 2000);
            }
          }, { tab_id: currentTab.id, args: [line.xpath] });
        },
      })),
    });
  }
  ...
  async function showTabPicker(
    ...
    filterFn?: (tab: browser.tabs.Tab) => boolean,
  ) {
    ...
    glide.commandline.show({
      title: `${actionTitles[action]} (${filtered.length} tabs)`,
        ...
        async execute() {
          ...
          if (action === "close") {
            ...
            if (currentTab.id === tab.id) {
              ...
              const nextTab =
                allTabs[currentIndex + 1] || allTabs[currentIndex - 1];
              if (nextTab?.id) {
  ...
  async function showHistoryPicker(currentSiteOnly: boolean = false) {
    const allHistory = await browser.history.search({
      text: "",
      startTime: 0,
      maxResults: currentSiteOnly ? 400 : 1000,
    });

    let filtered = allHistory;
    let title = `All History (${allHistory.length} items)`;
    ...
    glide.commandline.show({
      title,
      options: filtered.slice(0, 4000).map((item) => ({
  ...
  async function showSplitViewPicker(tab_id: number) {
    ...
    glide.commandline.show({
      title: "Select a tab to split with",
      options: all_tabs.map((tab) => ({
        ...
        async execute() {
          if (tab.id === tab_id) {
            glide.commandline.show({
              title: "You cannot split with the current tab.",
            });
            return;
          }
  ...
  // ============================================================================
  // CONTEXT MENU & REDIRECTS (credits: @poslat-pismo)
  // ============================================================================

  async function setupExtendedContextMenu() {
    if (!glide.prefs.get("network.protocol-handler.external.mpv")) {
      glide.prefs.set("network.protocol-handler.external.mpv", true);
    }
    ...
    browser.contextMenus.create({
      ...
      contexts: ["link"],
      onclick: (info) => {
        const mpvUrl = `mpv://#${info.linkUrl}`;
        browser.tabs.update({ url: mpvUrl });
      },
    });
    ...
    browser.contextMenus.create({
      ...
      onclick: (info) => {
        ...
        browser.notifications.create("tts", {
          type: "basic",
          title: "glide-say-TTS",
          message: selection,
        });
  ...
  const REDIRECT_RULES: Record<string, string> = {
    "^https?://(?:www\\.|old\\.|new\\.|np\\.)?reddit\\.com(.*)$":
      "https://libreddit.kavin.rocks$1",
  };
  ...
  function setupOptions() {
    ...
    glide.o.hint_label_generator = async ({ content }) => {
      ...
      const haystack = texts.map(
        ([text, label]) => stripText(text) || stripText(label || ""),
      );
      return generateLabels(haystack);
    };
  }
  ...
  function setupKeymaps() {
    ...
    // Link pickers
    glide.keymaps.set(ALL_MODES, "<C-.>", () => showClickablePicker("open"));
    glide.keymaps.set("normal", "<A-y>", () => showClickablePicker("copy"));
    glide.keymaps.set("normal", "<A-.>", () => showClickablePicker("newtab"));
    glide.keymaps.set("normal", "<A-:>", () => showClickablePicker('click'));

    glide.keymaps.set("normal", "<A-s>", () => showLineNavigator());
  ...
  function setupExcmds() {
    glide.excmds.create(
      {
        name: "tab_discard",
        description: "Discard (unload) a tab to free memory",
      },
      async ({ tab_id }) => {
        const currentTab = await glide.tabs.active();
        if (currentTab.id === tab_id) {
          await browser.tabs.update(currentTab.id, { active: false });
          const allTabs = await browser.tabs.query({ currentWindow: true });
          const currentIndex = allTabs.findIndex((t) => t.id === currentTab.id);
  ...
  function setupAutocommands() {
    ...
    glide.autocmds.create("ConfigLoaded", async () => {
      await setupExtendedContextMenu();
    });
    ...
    glide.autocmds.create("ModeChanged", "*", (args) => {
      ...
      glide.styles.add(
        `#browser { border: 1px solid var(${mode_colors[args.new_mode] ?? fallback}) }`,
